...
const currentMonthKey = () => `${$('month-select-year').value}-${$('month-select-month').value}`;
function populateMonthPicker(){
  $('month-select-month').innerHTML = MONTH_NAMES.map((name,i)=>`<option value="${String(i+1).padStart(2,'0')}">${name}</option>`).join('');
  const y=new Date().getFullYear(),years=[];for(let i=y-15;i<=y+15;i++)years.push(i);
  $('month-select-year').innerHTML = years.map(i=>`<option value="${i}">${i}</option>`).join('');
}
function setMonthKey(key){const [y,m]=key.split('-'); $('month-select-year').value=y; $('month-select-month').value=m}
...
function initMonth(){ populateMonthPicker(); const d=new Date(); setMonthKey(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`); $('transaction-date').value=today(); loadMonth(); }
...
function budgetRowHTML(k){const spent=expenseByCategory();const limit=app.budgets[k]||0,use=spent[k]||0,pct=limit?Math.round(use/limit*100):0,canDelete=Object.keys(app.budgets).length>1;return `<article class="budget-row ${limit&&use>limit?'budget-over':''}" data-budget-row="${esc(k)}"><div class="budget-head"><strong>${esc(k)}</strong><input class="currency-input budget-limit" data-budget="${esc(k)}" value="${limit?moneyText(limit):''}" placeholder="Sin límite" aria-label="Límite mensual para ${esc(k)}"></div><div class="budget-progress"><span style="width:${Math.min(pct,100)}%"></span></div><div class="budget-meta"><span>${money(use)} usados</span><span>${limit?`${pct}% de ${money(limit)}`:'Sin límite'} ${canDelete?`· <button class="mini-btn" data-delete-budget="${esc(k)}" aria-label="Eliminar categoría ${esc(k)}">Eliminar</button>`:''}</span></div></article>`}
...
function updateBudgetRow(key){const row=$('budget-list').querySelector(`[data-budget-row="${CSS.escape(key)}"]`);if(!row)return;const spent=expenseByCategory(),limit=app.budgets[key]||0,use=spent[key]||0,pct=limit?Math.round(use/limit*100):0,canDelete=Object.keys(app.budgets).length>1;row.classList.toggle('budget-over',!!(limit&&use>limit));row.querySelector('.budget-progress span').style.width=Math.min(pct,100)+'%';row.querySelector('.budget-meta').innerHTML=`<span>${money(use)} usados</span><span>${limit?`${pct}% de ${money(limit)}`:'Sin límite'} ${canDelete?`· <button class="mini-btn" data-delete-budget="${esc(key)}" aria-label="Eliminar categoría ${esc(key)}">Eliminar</button>`:''}</span>`}
...
function shiftMonth(delta){const [y,m]=currentMonthKey().split('-').map(Number),d=new Date(y,m-1+delta,1);setMonthKey(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`);inheritRecurring();loadMonth()}
...
['month-select-month','month-select-year'].forEach(id=>$(id).addEventListener('change',()=>{inheritRecurring();loadMonth()}));
